...
save_path = '../../DAC/BERT-DAC/ernie_predictions.npz'
...
def make_data(samples, mode='train', max_src_len=500, max_tgt_len=240):
    ...
    for pid, sample in samples.items():
        ...
        if mode == 'test':
            ...
            content = " ".join(tok.tokenize(content.strip()))
            title1 = " ".join(tok.tokenize(title1.strip()))
            title2 = " ".join(tok.tokenize(title2.strip()))
        if mode == 'test':
            src += content + '\n'
            tgt += title1 + '\n'
        else:
